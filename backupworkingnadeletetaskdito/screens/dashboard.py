...
class DashboardPage(tk.Frame):
    ...
    def build_navbar(self):
        ...
        nav.pack(fill='x')
        nav_pages = {'Home': 'DashboardPage', 'All Tasks': 'AllTasksPage', 'Calendar': 'CalendarPage',
    ...
    def build_right_panel(self):
        ...
        tk.Label(header_frame, text=f"{near_deadline_tasks} tasks near deadline", font=("Arial", 12),
    ...
    def delete_task(self, task):
        from database import delete_task, get_all_tasks
        confirm = tk.messagebox.askyesno("Delete Task", f"Are you sure you want to delete '{task['name']}'?")
        if confirm:
            delete_task(task["id"])
            user_id = self.controller.current_user["id"]
            self.controller.task_data = get_all_tasks(user_id)
