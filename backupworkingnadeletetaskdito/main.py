...
class App(tk.Tk):
    ...
    def on_login_success(self, user_data):
        ...
        # Refresh task data for the logged-in user
        self.task_data = database.get_all_tasks(user_data["id"])

        # Refresh Dashboard and AllTasksPage to reflect tasks
        if "DashboardPage" in self.frames:
            self.frames["DashboardPage"].refresh_dashboard()
        if "AllTasksPage" in self.frames:
            self.frames["AllTasksPage"].refresh_from_db()
